...
cd timetable-engine
gnome-terminal --tab --title="Python Engine" -- bash -c "python main_v20.py; exec bash" 2>/dev/null || \
xterm -T "Python Engine" -e "python main_v20.py; bash" 2>/dev/null || \
osascript -e 'tell app "Terminal" to do script "cd '"$(pwd)"' && python main_v20.py"' 2>/dev/null &
cd ..
sleep 3
