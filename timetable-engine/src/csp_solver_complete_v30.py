"""
Complete CSP Solver v3.0 - Simplified Room Allocation
...
class CSPSolverCompleteV30:
    """
    Complete CSP Solver v3.0 with simplified room allocation.

    KEY FEATURES:
    1. Greedy teacher pre-assignment (from v2.5.2)
    2. One teacher per subject per class (from v2.5.2)
    3. SIMPLIFIED ROOM ALLOCATION (new in v3.0):
            ...
            for slot_index, slot in enumerate(active_slots):
                ...
                for try_index in range(len(subjects_to_assign)):
                    ...
                    entry = TimetableEntry(
                        ...
                        } if hasattr(subject, 'prefer_morning') else None,
                        teacher_metadata={
                    ...
                    if is_shared:
                        shared_room_busy.add((room_id, slot.id))

                    # Update count
                    class_subject_count[(class_obj.id, subject.id)] = current_count + 1
                    assigned = True
                    break
