...
CHANGELOG v3.0.0:
- NEW: Simplified room allocation (2-level logic)
- NEW: Pre-assigned home classrooms (from database)
- NEW: Only schedules shared amenities (labs, sports, library, art)
- NEW: 85% reduction in room conflict checks
- NEW: Home classroom validation before generation
- Maintains all v2.5 features (teacher consistency, metadata, etc.)
- Integrated models_phase1_v30.py (simplified room models)
- Integrated csp_solver_complete_v30.py (2-level room allocation)
- Enhanced validation (V30Validator)
- Clearer error messages for room conflicts
...
- Special subjects → Allocate from shared amenities (WITH conflict check)

v3.0.0 ALGORITHM:
Phase 0: Validate home classrooms are assigned (NEW)
Phase 1: Greedy teacher assignment (unchanged)
Phase 2: CSP scheduling with simplified room allocation (NEW)
Phase 3: GA optimization (unchanged)
"""
...
# v3.0: Import updated models with simplified room logic
from src.models_phase1_v30 import (
    Class, Subject, Teacher, TimeSlot, Room,
    V30Validator, get_version_info
)

# v2.5 models still used for request/response (backward compatible)
...
# v3.0: Import simplified CSP solver
from src.csp_solver_complete_v30 import CSPSolverCompleteV30

# v2.5: GA optimizer (unchanged)
...
async def lifespan(app: FastAPI):
    ...
    print("\n" + "=" * 80)
    print(f">>> TIMETABLE GENERATION API v{version_info['version']} - STARTING UP")
    print("=" * 80)
    print(f"[*] Version Name: {version_info['version_name']}")
    print(f"[*] Release Date: {version_info['release_date']}")
    print("[*] SIMPLIFIED ROOM ALLOCATION:")
    print("    - Home classrooms pre-assigned in database")
    print("    - Only schedules shared amenities (labs, sports, library)")
    print("    - 85% reduction in room conflict checks")
    print("[*] Metadata-driven optimization enabled")
    print("[*] Teacher consistency enforcement")
    print("=" * 80 + "\n")

    # Initialize solvers globally (reused across requests)
    app.state.csp_solver = CSPSolverCompleteV30(debug=True)  # v3.0
    ...
    # Shutdown
    print("\n" + "=" * 80)
    print("<<< TIMETABLE GENERATION API v3.0 - SHUTTING DOWN")
    print("=" * 80 + "\n")
...
app = FastAPI(
    ...
    version="3.0.0",
    description="Simplified room allocation with metadata-driven optimization (CSP + GA)",
    lifespan=lifespan
)
...
async def root():
    ...
    return {
        ...
        "status": "operational",
        "features": {
            "simplified_room_allocation": True,  # v3.0 NEW
            "home_classroom_validation": True,   # v3.0 NEW
            "shared_amenity_scheduling": True,   # v3.0 NEW
            "metadata_driven": True,
            "language_agnostic": True,
            "school_customizable": True,
            "teacher_consistency": True,
            "csp_solver": "v3.0",
            "ga_optimizer": "v2.5"
        },
        "performance": {
            "room_conflict_reduction": "85%",     # v3.0 NEW
            "before_checks": "1,400 (40 rooms × 35 slots)",
            "after_checks": "210 (6 shared rooms × 35 slots)"
        },
        "endpoints": {
            "generate": "/generate",
            "validate": "/validate",
