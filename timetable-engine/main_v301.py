...
CHANGELOG v3.0.1:
- NEW: Cached lookups for hot paths
- NEW: Pre-computed subject metadata
- NEW: Reduced redundant calculations in nested loops
- NEW: Optimized teacher search
- NEW: Pre-shuffled subject assignments
- Maintains all v3.0 features (simplified room allocation)

PERFORMANCE IMPROVEMENTS FROM v3.0:
- Pre-compute subject metadata (avoid repeated hasattr())
- Cache frequently accessed values (slot.id, class.id)
- Pre-shuffle subjects once per class (not per slot)
- Optimized teacher search with early termination
- Reduced dictionary lookups in hot paths
...
- Home classroom validation before generation
...
v3.0.1 ALGORITHM:
Phase 0: Validate home classrooms are assigned
Phase 1: Greedy teacher assignment
Phase 2: CSP scheduling with optimized execution (NEW)
...
# v3.0.1: Import updated models with simplified room logic
...
# v3.0.1: Import performance-optimized CSP solver
...
async def lifespan(app: FastAPI):
    ...
    app.state.csp_solver = CSPSolverCompleteV301(debug=True)  # v3.0.1
